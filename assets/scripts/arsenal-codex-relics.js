...
function collectionCard(group,id,name,art,subtitle="",description=""){
 const unlocked=!!ensureDiscovery()[group][id];return `<article class="collection-card ${unlocked?"":"locked"}"><i><img src="${art}" alt=""></i><b>${unlocked?name:"???"}</b><span>${unlocked?subtitle:tr("尚未发现","UNDISCOVERED")}</span>${unlocked&&description?`<em>${description}</em>`:""}</article>`;
}
