...
update=function(dt){
 // Keep the simulation bounded before the source collision pass runs.  This is
 // especially important after an evolution adds extra projectiles; trimming at
 // the end of the frame is too late because collision checks already happened.
 if(shots.length>220)shots.splice(0,shots.length-220);
 if(particles.length>720)particles.splice(0,particles.length-720);
 if(weaponFx.length>110)weaponFx.splice(0,weaponFx.length-110);
 legacyUpdateV2(dt);
...
const recipeRule=r=>r.passive?tr("武器 LV.5 ＋ 配套物品 LV.1","WEAPON LV.5 + PARTNER ITEM LV.1"):tr("武器 LV.5","WEAPON LV.5");
...
`.replace(".pause-recipe-icons img{width:32px;height:32px;object-fit:contain;image-rendering:pixelated}",".pause-recipe-icons img{width:32px;height:32px;object-fit:contain;image-rendering:pixelated}.evo-icon{position:relative;display:inline-grid;place-items:center;width:38px;height:38px;border:2px solid #e8f4ff;background:radial-gradient(circle,#64758a,#1a202a);box-shadow:0 0 10px #cce8ff}.evo-icon img{width:28px!important;height:28px!important;filter:saturate(1.35) contrast(1.15)}.evo-icon em{position:absolute;right:-5px;top:-9px;color:#fff;font-size:14px;text-shadow:0 0 5px #a9d8ff;font-style:normal}.evolution-reward{border-color:#eef8ff!important;box-shadow:0 0 0 4px #63748b,0 0 24px 8px rgba(203,232,255,.9)!important;animation:evolutionCardPulse 1.2s steps(4,end) infinite!important}.evolution-reward #chestRewardIcon{filter:saturate(1.5) contrast(1.2);text-shadow:0 0 14px #d9edff}.evolution-reward #chestRewardName{color:#f4f9ff!important;text-shadow:0 0 8px #a9d8ff}@keyframes evolutionCardPulse{50%{box-shadow:0 0 0 4px #d9e8f7,0 0 38px 14px rgba(203,232,255,1)!important}}");document.head.appendChild(style);

// A ready recipe evolves on the next normal level-up, matching the elite-chest timing.
ui.choices?.addEventListener("click",()=>setTimeout(()=>{
 const ready=evolutionRecipes.find(r=>!evolved[r.weapon]&&evolutionReady(r));
 if(ready){evolved[ready.weapon]=true;discover("evolutions",ready.weapon);flash=Math.max(flash||0,.42);sfx("chestReveal");texts.push({x:player.x,y:player.y-58,text:tr(`究极${ready.name}`,`ULTIMATE ${ready.name}`),color:"#e8f4ff",life:1.25,crit:true,size:22});}
 renderTray();
},0),true);
...
showChestReward=function(reward){
 const isEvolution=!!(reward&&(reward.evolution||String(reward.icon||"").endsWith("Evolved")||/进化|Evolution|evolution/i.test(String(reward.desc||""))));
 if(isEvolution){reward.evolution=true;const apply=reward.apply;reward.apply=function(){apply();const recipe=evolutionRecipes.find(r=>evolved[r.weapon]&&!ensureDiscovery().evolutions[r.weapon]);if(recipe)discover("evolutions",recipe.weapon);};}
 const result=legacyShowChestRewardV2(reward),panel=document.querySelector("#chestScreen,.chest-screen,.chest-stage")||ui.chest;
 if(panel)panel.classList.toggle("evolution-reward",isEvolution);
 return result;
};
