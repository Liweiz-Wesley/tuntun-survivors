...
`.replace(".pause-recipe-icons img{width:32px;height:32px;object-fit:contain;image-rendering:pixelated}",".pause-recipe-icons img{width:32px;height:32px;object-fit:contain;image-rendering:pixelated}.evo-icon{position:relative;display:inline-grid;place-items:center;width:38px;height:38px;border:2px solid #e8f4ff;background:radial-gradient(circle,#64758a,#1a202a);box-shadow:0 0 10px #cce8ff}.evo-icon img{width:28px!important;height:28px!important;filter:saturate(1.35) contrast(1.15)}.evo-icon em{position:absolute;right:-5px;top:-9px;color:#fff;font-size:14px;text-shadow:0 0 5px #a9d8ff;font-style:normal}");document.head.appendChild(style);

// A ready recipe evolves on the next normal level-up, matching the elite-chest timing.
ui.choices?.addEventListener("click",()=>setTimeout(()=>{for(const r of evolutionRecipes){if(!evolved[r.weapon]&&evolutionReady(r)){evolved[r.weapon]=true;discover("evolutions",r.weapon);}}renderTray();},0),true);
