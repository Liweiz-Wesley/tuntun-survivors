...
function collectionCard(group,id,name,art,subtitle=""){
 const unlocked=!!ensureDiscovery()[group][id];return `<article class="collection-card ${unlocked?"":"locked"}"><i><img src="${art}" alt=""></i><b>${unlocked?name:"???"}</b><span>${unlocked?subtitle:tr("尚未发现","UNDISCOVERED")}</span></article>`;
}
function renderCodexV2(){
 ...
 scroll.innerHTML=`<div class="collection-summary"><b>${Object.values(ensureDiscovery()).reduce((n,g)=>n+Object.values(g).filter(Boolean).length,0)}</b><span>${tr("已收藏","DISCOVERED")}</span></div><h3>${tr("武器","WEAPONS")} · ${baseWeaponDefs.length}</h3><div class="collection-grid">${baseWeaponDefs.map(u=>collectionCard("weapons",u.id,u.name,pixelIconSrc(u.id),tr("最高 LV.8","MAX LV.8"))).join("")}</div><h3>${tr("被动加强","PASSIVES")} · ${passiveDefs.length}</h3><div class="collection-grid">${passiveDefs.map(u=>collectionCard("passives",u.id,u.name,pixelIconSrc(u.id),tr("改变基础属性","BASE STAT BOOST"))).join("")}</div><h3>${tr("融合形态","EVOLUTIONS")} · ${evoDefs.length}</h3><div class="collection-grid">${evoDefs.map(r=>collectionCard("evolutions",r.weapon,r.name,pixelIconSrc(r.icon),`${upgrades.find(u=>u.id===r.weapon)?.name||r.weapon} + ${upgrades.find(u=>u.id===r.passive)?.name||r.passive}`)).join("")}</div><h3>${tr("Boss 遗物","BOSS RELICS")} · ${allRelics.length}</h3><div class="collection-grid">${allRelics.map(r=>collectionCard("relics",r[0],r[1],foodRoot+"misc/shish_kabob.png",r[3])).join("")}</div><h3>${tr("怪物档案","BESTIARY")}</h3><div class="collection-grid monsters">${monsterCatalog.map(m=>collectionCard("enemies",m[0],tr(m[1],m[2]),m[3],"")).join("")}</div>`;
}
...
.collection-summary{display:flex;gap:10px;align-items:baseline;padding:10px 14px;background:#20213a;color:#fff;border:3px solid #e9cf77;box-shadow:inset 0 0 0 3px #58452c}.collection-summary b{font-size:30px;color:#ffe485}.collection-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(112px,1fr));gap:10px}.collection-card{min-height:132px;padding:10px 7px;background:#2a2940;border:3px solid #c9a85f;box-shadow:inset 0 0 0 2px #4f4250;color:#fff;text-align:center}.collection-card i{display:grid;place-items:center;width:56px;height:56px;margin:0 auto 6px;background:#171725;border:2px solid #806b49}.collection-card img{width:46px;height:46px;object-fit:contain;image-rendering:pixelated}.collection-card b,.collection-card span{display:block}.collection-card b{font-size:13px;color:#ffe7a1}.collection-card span{font-size:10px;color:#bbb5c5;margin-top:4px}.collection-card.locked i{background:#050508}.collection-card.locked img{filter:brightness(0);opacity:.9}.collection-card.locked{border-color:#514b58}.relic-draft{position:fixed;inset:0;z-index:70;display:none;place-items:center;background:rgba(12,10,20,.78)}.relic-draft.show{display:grid}.relic-panel{width:min(980px,94vw);padding:28px;background:#181827;border:4px solid #f0d477;box-shadow:0 0 0 5px #59452c,0 30px 80px #000;color:#fff;text-align:center}.relic-panel h2{color:#ffe58d}.relic-options{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}.relic-options button{min-height:260px;padding:20px;background:#28273a;border:3px solid #a99567;color:#fff;cursor:pointer}.relic-options button:hover{transform:translateY(-5px);border-color:#fff08f;box-shadow:0 0 24px #e7c94d}.relic-options i,.relic-options b,.relic-options em,.relic-options span{display:block}.relic-options i{font-size:48px;color:#ffe067}.relic-options b{font-size:24px;margin:12px 0 4px}.relic-options em{color:#c9b46e}.relic-options span{margin-top:20px;line-height:1.5}@media(max-width:720px){.relic-options{grid-template-columns:1fr}.relic-options button{min-height:150px}.relic-panel{max-height:92vh;overflow:auto}}
